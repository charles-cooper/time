...
fromOrdinalDate :: Int32 -> Int32 -> Day
fromOrdinalDate year day = ModifiedJulianDay mjd where
    y = year - 1
    mjd = (clip 1 (if isLeapYear year then 366 else 365) day) + (365 * y) + (div y 4) - (div y 100) + (div y 400) - 678576

-- | Convert from ISO 8601 Ordinal Date format.
-- Invalid day numbers return 'Nothing'
fromOrdinalDateValid :: Int32 -> Int32 -> Maybe Day
...
mondayStartWeek :: Day -> (Int32,Int32)
mondayStartWeek date = ((div d 7) - (div k 7),(mod d 7) + 1) where
...
sundayStartWeek :: Day -> (Int32,Int32)
sundayStartWeek date =((div d 7) - (div k 7),mod d 7) where
    yd = snd (toOrdinalDate date)
...
fromMondayStartWeek :: Int32 -- ^ Year.
                    -> Int32 -- ^ Monday-starting week number (as @%W@ in 'Data.Time.Format.formatTime').
                    -> Int32 -- ^ Day of week.
                             -- Monday is 1, Sunday is 7 (as @%u@ in 'Data.Time.Format.formatTime').
                    -> Day
fromMondayStartWeek year w d = let
    ...
    -- 0-based day in year
    zbYearDay = zbFirstMonday + 7 * zbWeek + zbDay

    in addDays zbYearDay firstDay

fromMondayStartWeekValid :: Int32 -- ^ Year.
                    -> Int32 -- ^ Monday-starting week number (as @%W@ in 'Data.Time.Format.formatTime').
                    -> Int32 -- ^ Day of week.
                             -- Monday is 1, Sunday is 7 (as @%u@ in 'Data.Time.Format.formatTime').
                    -> Maybe Day
...
fromSundayStartWeek :: Int32 -- ^ Year.
                    -> Int32 -- ^ Sunday-starting week number (as @%U@ in 'Data.Time.Format.formatTime').
                    -> Int32 -- ^ Day of week
                             -- Sunday is 0, Saturday is 6 (as @%w@ in 'Data.Time.Format.formatTime').
                    -> Day
fromSundayStartWeek year w d = let
    ...
    -- 0-based day in year
    zbYearDay = zbFirstSunday + 7 * zbWeek + zbDay

    in addDays zbYearDay firstDay

fromSundayStartWeekValid :: Int32 -- ^ Year.
                         -> Int32 -- ^ Sunday-starting week number (as @%U@ in 'Data.Time.Format.formatTime').
                         -> Int32 -- ^ Day of week.
                                  -- Sunday is 0, Saturday is 6 (as @%w@ in 'Data.Time.Format.formatTime').
                         -> Maybe Day
