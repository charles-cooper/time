...
toWeekDate :: Day -> (Int32,Int32,Int32)
toWeekDate date@(ModifiedJulianDay mjd) = (y1,w1 + 1, d_mod_7 + 1) where
...
fromWeekDate :: Int32 -> Int32 -> Int32 -> Day
fromWeekDate y w d = ModifiedJulianDay (k - (mod k 7) + (((clip 1 (if longYear then 53 else 52) w) * 7) + (clip 1 7 d)) - 10) where
...
fromWeekDateValid :: Int32 -> Int32 -> Int32 -> Maybe Day
fromWeekDateValid y w d = do
    ...
    let
        k = toModifiedJulianDay (fromOrdinalDate y 6)
    return (ModifiedJulianDay (k - (mod k 7) + ((w' * 7) + d') - 10))
